...
epochs = 20
batch_size = 32
train_dir = 'C:\\Users\\USER\\Desktop\\data_2\\model3\\train\\'
valida_dir = 'C:\\Users\\USER\Desktop\\data_1\\model3\\test\\'
...
def model_create(model_input):
    ...
    model = Model(model_input, x, name='nin_cnn3')
    return model

def compile_and_train(model, num_epochs):

    model.compile(loss=categorical_crossentropy, optimizer=Adam(lr=1e-4), metrics=['acc'])
    filepath = 'weights3/' + model.name + '.{epoch:02d}-{loss:.2f}.hdf5'
    checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=0, save_weights_only=True,
                                 save_best_only=True, mode='auto', period=1)
    tensor_board = TensorBoard(log_dir='logs3/', histogram_freq=0, batch_size=batch_size)
    history = model.fit(x=X_train, y=y_train, batch_size=batch_size,
                        epochs=num_epochs, verbose=1, callbacks=[checkpoint, tensor_board],validation_data=(X_val,y_val))
    return history
...
model3 = model_create(model_input)
_ = compile_and_train(model3, num_epochs=epochs)
err = evaluate_error(model3)
print('error',err)
print('acc',1-err)
loss,acc = model3.evaluate(X_test,y_test0)
