...
def image_to_array_0(j):
    ...
    :return:
    """
    image_base_path = "C:\\Users\\USER\\Desktop\\experiment_data\\model1\\train\\{}\\".format(j)
    print(image_base_path)

    filenames_0 = os.listdir("C:\\Users\\USER\\Desktop\\experiment_data\\model1\\train\\{}\\".format(j))
    n = len(filenames_0) # 获取图片的个数
    result = np.array([])  # 创建一个空的一维数组
    label = np.empty((num2,),dtype="int")
